...
#     print('Loaded GloVe!')
#     file.close()
#     embedding_dim = len(vocab.values()[0])
#     vocab['<unk>'] = [0.0]*embedding_dim
...
def load_glove(filename):
    ...
    file.close()
    embedding_dim = len(vocab.values()[0])
    embeddings[0] = [0.0]*embedding_dim
    embeddings[1] = [0.0]*embedding_dim
    return vocab, embeddings, embedding_dim


# def get(vocab, texts):  # 好像很慢，考虑还是搞index往里放
#     words = []
#
...
#         for lemma in lemmas:
#             if lemma in vocab:
#                 words_embedding.append(vocab[lemma])
#             else:
#                 words_embedding.append(vocab['<unk>'])  # <unk>
#         words.append(words_embedding)
#
#     # print maxlen
#
#     return words
...
def get_batch(inputs_q, inputs_p, passage_numbers, passage_word_numbers,
    ...
    inputs_q_padding_batch = []
    inputs_p_padding_batch = []
    ...
    if batch_res != 0:  # 最后一个暂时不要了；不对啊，test不能不要，咋办；补个齐？？
        ...
        if is_test is False:
            for i in range(batch_pad):
                ...
                inputs_q_padding_batch.append(inputs_q[no])
                inputs_p_padding_batch.append(inputs_p[no])
            ...
            for i in range(batch_pad):
                inputs_q_padding_batch.append([0])
                inputs_p_padding_batch.append([0])
    ...
    if is_test is False:
        ...
        for i in range(data_num):
            record = [inputs_q[i], inputs_p[i], passage_numbers[i], passage_word_numbers[i],
    ...
    for batch_no in range(batch_num):
        ...
        if len(inputs_q_feed) < batch_size:
            ...
            inputs_q_feed += inputs_q_padding_batch
        ...
        if len(inputs_p_feed) < batch_size:
            # print len(encoder_inputs_q_feed)
            inputs_p_feed += inputs_p_padding_batch
        ...
        passage_word_number_feed = padding(passage_word_number_feed)  # padding
        ...
        if is_test is False:
            ...
            passage_ys_feed = padding(passage_ys_feed)  # padding
...
def train(model, sess, train_inputs_q, train_p, train_passage_numbers, train_passage_word_numbers,
      train_starts, train_ends, train_passage_ys, embeddings):
    losses = []
    ...
    batch_passage_numbers, batch_passage_word_numbers, batch_starts, batch_ends, batch_passage_ys\
        = get_batch(train_inputs_q, train_p, train_passage_numbers, train_passage_word_numbers, False,
      train_starts, train_ends, train_passage_ys)

    # tvars = tf.trainable_variables()
    # print tvars
    # grads, _ = tf.clip_by_global_norm(tf.gradients(model.loss, tvars), grad_clip)
    ...
    for epoch in range(epoch_num):
        for batch_no in range(len(batch_pad_inputs_q)):  # len(batch_pad_inputs_q)
            feed_dict = {model.inputs_q: batch_pad_inputs_q[batch_no],
                         model.inputs_actual_length_q: batch_inputs_q_actual_lengths[batch_no],
                         model.inputs_concat_p: batch_pad_inputs_p[batch_no],
...
def test(model, sess, test_q, test_p, test_passage_numbers, test_passage_word_numbers, embeddings):
    starts_one_hot = []
    ends_one_hot = []

    batch_pad_inputs_q, batch_inputs_q_actual_lengths, batch_pad_inputs_p, batch_inputs_p_actual_lengths, \
    batch_passage_numbers, batch_passage_word_numbers, batch_starts, batch_ends, batch_passage_ys \
        = get_batch(test_q, test_p, test_passage_numbers, test_passage_word_numbers, True)
    for batch_no in range(len(batch_pad_inputs_q)):  # len(batch_pad_inputs_q)
        feed_dict = {model.inputs_q: batch_pad_inputs_q[batch_no],
                     model.inputs_actual_length_q: batch_inputs_q_actual_lengths[batch_no],
                     model.inputs_concat_p: batch_pad_inputs_p[batch_no],
...
print 'load glove...'
vocab, embeddings, embedding_dim = load_glove(glove_filepath)
ext_vocab_size = len(embeddings)

print 'build extraction model...'
train_model = ExtractionModel(batch_size, hidden_size, embedding_dim, dropout_rate, grad_clip, initial_learning_rate, ext_vocab_size)
test_model = ExtractionModel(batch_size, hidden_size, embedding_dim, dropout_rate, grad_clip, initial_learning_rate, ext_vocab_size,  'test')
...
train_inputs_q = to_index(vocab, train_queries)
train_inputs_p = to_index(vocab, train_concat_passages)
print 'train...'
train(train_model, sess, train_inputs_q, train_inputs_p, train_passage_numbers, train_passage_word_numbers,
      train_start_positions, train_end_positions, train_passage_ys, embeddings)
...
dev_inputs_q = to_index(vocab, dev_queries)
dev_inputs_p = to_index(vocab, dev_concat_passages)
print 'test (with dev data)...'
dev_starts_one_hot, dev_ends_one_hot = test(test_model, sess, dev_inputs_q, dev_inputs_p, dev_passage_numbers,
                                    dev_passage_word_numbers, embeddings)
...
test_inputs_q = to_index(vocab, test_queries)
test_inputs_p = to_index(vocab, test_concat_passages)
print 'test...'
test_starts_one_hot, test_ends_one_hot = test(test_model, sess, test_queries, test_concat_passages, test_passage_numbers,
                                    test_passage_word_numbers, embeddings)
