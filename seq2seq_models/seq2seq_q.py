...
# 训练
def train(model, train_inputs, train_targets, batch_size, epoch_num):
    losses = []
    ...
    for epoch in range(epoch_num):
        ...
        for batch_no in range(len(batch_pad_inputs)):  # len(batch_pad_inputs)
            ...
            print epoch, batch_no, ' loss: ', loss_value


# 测试
def test(model, test_inputs, test_targets, batch_size):
    all_sample_id = []

    batch_pad_inputs, batch_pad_targets, inputs_actual_lengths, targets_actual_lengths \
        = get_batch(test_inputs, test_targets, batch_size)

    print len(batch_pad_inputs)

    for batch_no in range(len(batch_pad_inputs)):  # len(batch_pad_inputs)
        feed_dict = {model.encoder_inputs: batch_pad_inputs[batch_no],
                     model.encoder_inputs_actual_length: inputs_actual_lengths[batch_no],
...
print 'train...'
train(train_model, train_inputs, train_targets, batch_size, epoch_num)
del train_inputs, train_targets

print 'read dev texts...'
dev_data_list = read_json(dev_path)
dev_query_ids, dev_queries = get_ms_marco_qa(dev_data_list, True)
...
dev_output_ids = test(test_model, dev_inputs, None, batch_size)
...
print 'test...'
test_output_ids = test(test_model, test_inputs, None, batch_size)
...
# 还需要dev的loss看看是否过拟合，所以回头还是改下，看是改模型还是改外面，另外get_batch等也要相应改